...
export function parseImage(msg: ToolMessage): ImageResponse {
    ...
    if (!payload || typeof payload !== "object") throw new Error("Invalid tool payload for generate_image");
    const obj = payload as UnknownRecord

    const url = "url" in obj && obj.url && typeof obj.url === "string" ? obj.url : ""

    return {
        url: url
    }
}
export function RenderImage({ payload,
}: RenderPreviewProps<ImageResponse>) {
